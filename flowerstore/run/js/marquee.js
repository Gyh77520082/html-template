...
* Plugin : http://www.jqueryscript.net/animation/jQuery-Plugin-For-Horizontal-Text-Scrolling-Simple-Marquee.html
// Structure //
  *********************************** - marque-container - *************************************
  *                                                                                            *
  *   ******************************* ******************************************************   *
...
function createMarquee(settings) {
	...
	if ($(config.marquee_class).width() == 0) {
		console
				.error('FATAL: marquee css or children css not correct. Width is either set to 0 or the element is collapsing. Make sure overflow is set on the marquee, and the children are postitioned relatively');
		return;
	}
	...
	for (i = 0; i < spawnAmount; i++) {
		...
		marqueeSpawned[i].currentPos = (widthToIgnore + (contentWidth * i))
				+ (config.padding * i); // initial positioning
		...
		marqueeSpawned[i].el.css('left', marqueeSpawned[i].currentPos
				+ config.padding + 'px'); // setting left according to
											// postition
...
function marqueeManager(marqueed_el) {
	...
	if (marqueed_el.hovered == false) {
		...
		if (marqueed_el.counter > 0) { // this is not the first loop
			...
			marqueed_el.el.css('left', marqueed_el.containerWidth + 'px'); // setting
																			// margin
