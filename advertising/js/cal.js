...
calObj[0].daysClass["10/14"] = 'Holyday;体育の日';
...
calObj[0].month["2-1-1"] = "Holyday;成人の日";
calObj[0].month["3-4-3"] = "Holyday;春分の日";
calObj[0].month["4-1-5"] = "Holyday;昭和の日";
calObj[0].month["4-2-5"] = "Holyday;国民の休日";
calObj[0].month["5-3-1"] = "Holyday;皇太子さま即位の日";
calObj[0].month["5-4-1"] = "Holyday;国民の休日";
calObj[0].month["5-5-1"] = "Holyday;憲法記念日";
calObj[0].month["5-6-1"] = "Holyday;みどりの日";
calObj[0].month["5-0-1"] = "Holyday;こどもの日";
calObj[0].month["5-1-1"] = "Holyday;振替休日";
calObj[0].month["7-1-3"] = "Holyday;海の日";
calObj[0].month["8-1-2"] = "Holyday;振替休日";
calObj[0].month["9-1-3"] = "Holyday;敬老の日";
calObj[0].month["9-1-4"] = "Holyday;秋分の日";
// calObj[0].month["10-1-2"] = "Holyday;体育の日";
calObj[0].month["11-2-1"] = "Holyday;文化の日";
calObj[0].month["11-1-4"] = "Holyday;勤労感謝の日";
...
calObj[3].defaultMonth = 1; //翌月から表示
// カレンダー3の設定はここまで
...
calObj.date = new Date(calObj.date.getFullYear() + "/" + (calObj.date.getMonth() + 1) + "/" + calObj.date.getDate() +
	" 00:00:00");
...
function cal_init() {
	...
	for (var i = 0; i < tagObj.length; i++) {
		if (tagObj[i].className == "cal_wrapper") {
			...
			if (0 < calObj.month + calObj[calId].defaultMonth && calObj.month + calObj[calId].defaultMonth < 13) calObj[calId].currentMonth =
				new Date(calObj.year + "/" + (calObj.month + calObj[calId].defaultMonth) + "/" + "1 00:00:00");
			else calObj[calId].currentMonth = new Date(calObj.year + 1 + "/" + ((calObj.month + calObj[calId].defaultMonth) % 12) +
				"/" + "1 00:00:00");
			cal_create(calId);
		}
	}
...
function cal_create(calId) {
	...
	for (dayCnt = 1; dayCnt <= calObj.days[month] + bisDay; dayCnt++) {
		...
		// backward
		if (calObj[calId].backward != null && currentDayDate.getTime() < calObj.date.getTime()) dayClass["backward"] = calObj[
			calId].backward;
		...
		if (calObj[calId].month[weekStr] != undefined) dayClass["week"] = calObj[calId].month[weekStr];
			...
			} else if (calObj[calId].week[week] != undefined && typeof calObj[calId].week[week] != "object") dayClass["week"] =
				calObj[calId].week[week];
		}
		// xDay
		...
		for (var ci = 0; ci < calObj[calId].priority.length; ci++) {
			if (dayClass[calObj[calId].priority[ci]] != undefined) {
				...
				if (splitArr[1] != undefined) {
					...
					tdTextListArr.push('<ol><li id="' + tdId +
						'_li" onmouseover="cal_list2day_over(this)" onmouseout="cal_list2day_out(this)" value="' + dayCnt + '"' +
						tdTextListLink + ">" + splitArr[1] + "</li></ol>");
				}
...
function cal_move(calId, m) {
	if (m == null) calObj[calId].currentMonth = new Date(calObj.year + "/" + calObj.month + "/" + "1 00:00:00");
		...
		var month = calObj[calId].currentMonth.getMonth() + 1;
		var year = calObj[calId].currentMonth.getFullYear();
		if (0 < month + m && month + m < 13) calObj[calId].currentMonth = new Date(year + "/" + (month + m) + "/" +
			"1 00:00:00");
		else if (month + m < 1) {
			year--;
			month = 12;
